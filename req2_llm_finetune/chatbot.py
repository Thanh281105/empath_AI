...
async def vertex_chat(messages: list[dict]) -> str:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            ...
            # rawPredict trả về direct response từ container
            if "reply" in result:
                return result["reply"]
            if "choices" in result and result["choices"]:
                return result["choices"][0].get("message", {}).get("content", "")
            # Fallback cho :predict format
...
async def groq_chat(messages: list[dict]) -> str:
    ...
    for attempt in range(max_retries):
        async with aiohttp.ClientSession() as session:
            async with session.post(GROQ_API_URL, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=60)) as resp:
                if resp.status == 429:
                    # Rate limit hit, wait and retry
                    wait_time = 2 * (attempt + 1)
                    print(f"  [Groq 429] Hết hạn mức, thử lại sau {wait_time}s...")
    ...
    raise RuntimeError("Groq API quá tải sau nhiều lần thử lại.")
