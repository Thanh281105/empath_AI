...
def _get_query_engine():
    ...
    from indexing.query_engine import QueryEngine
    return QueryEngine(top_k_search=TOP_K_SEARCH, top_k_rerank=TOP_K_RERANK)
...
async def _run_rag(question: str, use_vertex: bool) -> tuple[str, list[str]]:
    ...
    docs  = await loop.run_in_executor(None, engine.retrieve_and_rerank, question)

    retrieved_policy_ids: list[str] = [
        d.metadata.get("policy_id", d.metadata.get("doc_id", "")) for d in docs
    ]
